...
export function likeProductReviews () {
  return async (req: Request, res: Response, next: NextFunction) => {
    const id = mongoSanitize(sanitize(sanitize(req.body.id)))
    ...
    try {
      const review = await db.reviewsCollection.findOne({ _id: mongoSanitize(id) })
      if (!review) {
      ...
      await sleep(150)
      try {
        const updatedReview: Review = await db.reviewsCollection.findOne({ _id: mongoSanitize(id) })
